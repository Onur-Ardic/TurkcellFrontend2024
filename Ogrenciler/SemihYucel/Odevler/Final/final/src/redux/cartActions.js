import { setCartItems, removeItemFromCart as removeItem } from '@/redux/slices/cartSlice';
  ...
  if (user) {
    ...
    if (userCart) {
      const itemsWithDetails = userCart.items.map(item => {
        const product = products.products.find(product => product.id === parseInt(item.product_id));
        return {
          ...item,
          ...product
        };
...
export const saveCartItems = (items) => async dispatch => {
  const user = auth.currentUser;
  if (user) {
    const userCartIndex = products.boxes.findIndex(cart => cart.user_id === user.uid);
...
export const removeItemFromCart = (productId) => async dispatch => {
  ...
  if (user) {
    ...
    const userCartIndex = products.boxes.findIndex(cart => cart.user_id === user.uid);
    if (userCartIndex > -1) {
      products.boxes[userCartIndex].items = products.boxes[userCartIndex].items.filter(item => item.product_id !== productId);
