...
function fillModal(movieName) {
  ...
  const movie = movieStorage.movieNameList.find((movie) => movie.name === movieName)
  movieNameToUpdate = movie.name
  Page.FillModal(movie.name, movie.director, movie.year, movie.category, movie.image)
}
...
Storage.prototype.AddMovie = function (movie) {
  ...
  Page.AddCard(movie.name, movie.image, movie.year, movie.director, movie.category)
}
...
document.getElementById('movienameFilter').addEventListener('input', function () {
