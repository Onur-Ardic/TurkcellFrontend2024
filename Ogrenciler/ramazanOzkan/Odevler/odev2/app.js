...
class Book {
    constructor(id, title, author, price, category, publicationYear, upDate, updateUser,img) {
      this.id = id;
      this.title = title;
      this.author = author;
      this.price = price;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  saveBookButton.addEventListener('click', function() {
    var bookData = new Book(
      ...
      new Date(), // upDate
      'currentUser', // updateUser, you can replace 'currentUser' with actual user info
    ...
    Request.post('http://localhost:3000/books', bookData)
      ...
      .catch(err => alert('Kitap eklenirken hata oluştu:', err));
  });
  ...
  function openUpdateModal(book) {
    ...
    saveBookButton.onclick = function() {
      updateBook(book.id);
    };
  ...
  function updateBook(id) {
    const bookData = new Book(
      ...
      document.getElementById('publicationYear').value,
      new Date(), // upDate
      'currentUser', // updateUser, replace 'currentUser' with actual user info
      document.getElementById('bookImg').value
    );
  
    Request.put(`http://localhost:3000/books/${id}`, bookData)
      .then(response => {
        console.log('Kitap başarıyla güncellendi:', response);
        $('#bookModal').modal('hide');
        document.getElementById('bookForm').reset();
        document.getElementById('bookImgPreview').style.display = 'none';
        loadBooks(); // Update the book list after updating the book
      })
      .catch(err => alert('Kitap güncellenirken hata oluştu:', err));
  }
