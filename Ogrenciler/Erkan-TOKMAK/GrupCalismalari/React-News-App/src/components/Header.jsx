...
import { Navbar, NavText, NavbarItem, Container, Currency } from '../style/style.module'
import { NavLink } from "react-router-dom";
import './Header.css'
import Search from './Search';

const Header = ({search, handleSearch, getDataBySearch,currency,loading}) => {
  return (
    <>
    <Navbar>
      <Container>
        <NavbarItem>
          ...
          <Search search={search} handleSearch={handleSearch} getDataBySearch={getDataBySearch}/>
        </NavbarItem>
      </Container>
    </Navbar>
    <Currency>
    <marquee>{!loading && currency.length > 0 && console.log(currency) /*currency.map((item)=> <p>{item.BTCBRL.code}</p>)*/}  </marquee>
    </Currency>
    </>
  )
}
