...
const faktoriyel = sayi => (sayi == 0 || sayi == 1) ? 1 : sayi * faktoriyel(sayi - 1);
const isPrime = sayi => {
    if (sayi <= 1) return `Sayı Asal Değil: ${sayi}`;
    for (let i = 2; i <= sayi / 2; i++) if (sayi % i == 0) return `Sayı Asal Değil: ${sayi}`;
    return `Sayı Asal: ${sayi}`
}
const isSuper = sayi => {
    let result = 1;
    if (sayi <= 1) return 'Bu bir süper sayı değil.'
    for (let i = 2; i <= sayi / 2; i++) if (sayi % i == 0) result += i;
    return result == sayi ? 'Bu bir süper sayı.' : 'Bu bir süper sayı değil.'
}
while (true) {
...
4-) Süper`);
    ...
    switch (operationValue) {
        case "1":
            alert(`Faktöriyelin Sonucu (${userValue}): ${faktoriyel(userValue)}`);
            break;
        case "2":
