...
displayFormData(image, bookname, author, date, categories);

function displayFormData(image, bookname, author, date, categories) {
  const formDataContainer = document.getElementById("formDataContainer");
  formDataContainer.innerHTML = `
    <h2>Form Bilgileri</h2>
    <p><strong>Resim:</strong> image${image}</p>
    <p><strong>Kitap Adı:</strong> bookname${bookname}</p>
    <p><strong>Yazar:</strong> author${author}</p>
    <p><strong>Yayın Tarihi:</strong>date: ${date}</p>
    <p><strong>Kategoriler:</strong>category: ${categories}</p>
  `;
}
