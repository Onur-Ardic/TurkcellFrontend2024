...
class Film extends BaseFilm {
  constructor(filmAdi, yonetmen, yil, tur, afisUrl) {
    super(filmAdi, yonetmen, yil, tur, afisUrl);
  }
}

document.addEventListener("DOMContentLoaded", function () {
  const film = new Film(); // Temel film nesnesi yaratılıyor.
  film.renderFilmCollection(); // Sayfa yüklendiğinde koleksiyonu güncelle.
});
...
filmForm.addEventListener("submit", function (event) {
  ...
  localStorage.setItem("filmListesi", JSON.stringify(filmListesi));
  filmForm.reset();
  film.renderFilmCollection(); // Koleksiyonu güncelle.
});
